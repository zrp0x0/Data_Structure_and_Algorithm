...
### 10. 재귀 vs 반복문 트레이드 오프
- 재귀 고려
    - 데이터의 깊이가 깊지 않음이 확신할 때
    - 트리나 그래프 탐색처럼 문제 구조 자체가 재귀적일 때
    - 성능 최적화보다 개발 속도와 코드 명확성이 더 중요할 때

- 반목문(또는 명시적 스택)을 고려할 때
    - 데이터 양이 너무 많아 스택 오버플로우가 우려될 때
    - 초당 수만 번 실행되어야하는 극도로 민감한 성능이 요구되는 구간일 때
